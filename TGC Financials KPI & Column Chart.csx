// Title: Create Comparison Measures (Updated)
// Description: Creates a set of standard variance and KPI measures for a selected 'Act' measure.
...
VAR __budget = {budMeasureDaxName}
VAR __variance = __actual - __budget
VAR __variance_percent = DIVIDE(__variance, __budget)
VAR __format_percent = FORMAT(__variance_percent, ""#,0%;(#,0%)"")
VAR __sentiment =
    SWITCH(
        TRUE(),
...
VAR __format_string = SELECTEDVALUE('Format Strings Kk'[Format String], ""#,##0;(#,##0);0"")
VAR __format_number = FORMAT(__number, __format_string)
...
VAR __ly = {lyMeasureDaxName}
VAR __variance = __actual - __ly
VAR __variance_percent = DIVIDE(__variance, __ly)
VAR __format_percent = FORMAT(__variance_percent, ""#,0%;(#,0%)"")
VAR __sentiment =
    SWITCH(
        TRUE(),
